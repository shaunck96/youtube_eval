...
def thumbnail_extractor(video_id):
  # URL of the image
  image_url = "http://img.youtube.com/vi/%s/0.jpg" % video_id
  ...
  if response.status_code == 200:
    folder_path = '/content/thumbnails'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    # Set the filename including the folder path
    filename = os.path.join(folder_path, f"{video_id}.jpg")
